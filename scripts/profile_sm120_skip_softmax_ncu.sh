...
export FLASHINFER_WORKSPACE_BASE="${JIT_CACHE_BASE:-${repo_parent}/.flashinfer-sm120-ncu-jit}"
result_base="${RESULT_BASE:-${repo_parent}/sm120-skip-ncu-results}"
...
for mode in dense skip_no_tiles skip_active; do
  ...
  CUDA_VISIBLE_DEVICES="${gpu_id}" PYTHONPATH="${validation_pythonpath}" \
    "${ncu_bin}" \
      ...
      "${section_args[@]}" \
      --metrics "${metrics_csv}" \
      "${base_python}" "${repo_root}/benchmarks/profile_sm120_skip_softmax_ncu.py" \
      --mode "${mode}"
...
echo "SM120_SKIP_SOFTMAX_NCU: PASS"
