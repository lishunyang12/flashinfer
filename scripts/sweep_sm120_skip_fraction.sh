...
gpu_id="${GPU_ID:-4}"
numa_node=$((gpu_id / 4))
runtime_ref="${RUNTIME_REF:-origin/sm120-prims-skip-softmax-pr}"
...
echo "RUNTIME_REF_COMMIT=$(git -C "${repo_root}" rev-parse "${runtime_ref}")"
echo "GPU_ID=${gpu_id}"
...
if ! git -C "${repo_root}" diff --quiet "${runtime_ref}" HEAD -- "${runtime_files[@]}"; then
  echo "ERROR: performance branch runtime differs from ${runtime_ref}" >&2
  exit 2
fi
